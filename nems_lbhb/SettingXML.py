...
Edited by Charlie Heller 06/27/2019 for use in LBHB
"""
        ...
        for SubEl in El:
            if SubEl.keys():
                if SubEl.get('name'):
                    ...
                    # added for cases were multiple channels
                    if SubEl.tag in Dict:
                        newkey = SubEl.tag + str(ch_count)
                        ch_count+=1
                ...
                # Added to deal with multiple instances of the same El -
                # for example, if there are multiple signal chains
                if SubEl.tag in Dict:
                    Dict[SubEl.tag+'_'] = Root2Dict(SubEl)
                else:
                    Dict[SubEl.tag] = Root2Dict(SubEl)
        return(Dict)
    else:
...
def GetRecChs(File):
    ...
    # CRH added 'CHANNEL_INFO' in Info['SIGNALCHAIN']['PROCESSOR'][SourceProc]
    # to prevent error if CHANNEL_INFO not in dict
    if 'CHANNEL_INFO' in Info['SIGNALCHAIN']['PROCESSOR'][SourceProc] and \
                    Info['SIGNALCHAIN']['PROCESSOR'][SourceProc]['CHANNEL_INFO']:
