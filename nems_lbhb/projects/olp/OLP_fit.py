...
def calc_psth_metrics(batch, cellid, parmfile=None, paths=None):
    ...
    for cnt, stimmy in enumerate(ep_twostim):
        ABepo = resp.extract_epoch(stimmy)[:, :, presil:-postsil]
        kind = ohel.label_pair_type(stimmy)
        seps = (stimmy.split('_')[1], stimmy.split('_')[2])
        BG, FG = seps[0].split('-')[0][2:], seps[1].split('-')[0][2:]

        Aepo = resp.extract_epoch('STIM_' + seps[0] + '_null')[:, :, presil:-postsil]
        Bepo = resp.extract_epoch('STIM_null_' + seps[1])[:, :, presil:-postsil]

        A_FR, B_FR, AB_FR = np.nanmean(Aepo), np.nanmean(Bepo), np.nanmean(ABepo)

        min_rep = np.min((Aepo.shape[0], Bepo.shape[0])) #only will do something if SoundRepeats==Yes
