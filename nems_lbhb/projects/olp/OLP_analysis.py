...
def psth_comp_figure(resp_idx, pair, unit, response, params, sigma=None, error=True):
    """Produces a single PSTH with however many lines based on the number of indices
    given in resp_idx. Sigma adds smoothing, z adds a second subplot showing the zscore
    between the indices indicated in resp_idx, sum being true shows the linear sum in a
    dotted line on the psth."""

    ##This plot is to make the psth+spec+z for my SFN/APAN poster 10/2021
    ...
    # psth = plt.subplot2grid((9, 3), (3, 0), rowspan=3, colspan=6)
    # specA = plt.subplot2grid((9, 3), (0, 0), rowspan=1, colspan=6)
    # specB = plt.subplot2grid((9, 3), (1, 0), rowspan=1, colspan=6)
    # zz = plt.subplot2grid((9, 3), (6, 0), rowspan=3, colspan=6)
    # ax = [specA, specB, psth, zz]
    ...
    zz = plt.subplot2grid((18, 3), (9, 0), rowspan=5, colspan=6)
    ax = [specA, specB, psth, zz]
    ...
    for col, idx in zip(colors, resp_idx):
        ...
        x = np.linspace(0,response.shape[-1]/params['fs'],response.shape[-1]) - params['PreStimSilence']
        if sigma:
            ax[2].plot(x, sf.gaussian_filter1d(mean_resp, sigma) * params['fs'], color=col,
                    label=f"{tags[idx]}")
        if not sigma:
            ax[2].plot(x, mean_resp * params['fs'], color=col, label=f"{tags[idx]}")
        if error:
            sem = stats.sem(resp, axis=0, nan_policy='omit')
            ax[2].fill_between(x, sf.gaussian_filter1d((mean_resp - sem) * params['fs'], sigma),
                            sf.gaussian_filter1d((mean_resp + sem) * params['fs'], sigma),
                            alpha=0.5, color=col)
    ...
    mean_resplin = resplin.mean(axis=0)
    if sigma:
        ax[2].plot(x, sf.gaussian_filter1d(mean_resplin * params['fs'], sigma), color='dimgray',
                ls='--', label='Linear Sum')
    if not sigma:
        ax[2].plot(x, mean_resplin * params['fs'], color='dimgray', ls='--', label='Linear Sum')

    ax[2].set_xlim(-0.2, (params['Duration'] + 0.3))        # arbitrary window I think is nice
    # ax[2].set_xticks([0, (params['Duration'] / 2), params['Duration']])
    ax[2].set_xticks([])
    ymin, ymax = ax[2].get_ylim()

    ax[2].set_ylabel('spk/s', fontweight='bold', size=10)
    ax[2].legend(loc='upper right', fontsize=6)
    ax[2].vlines([0, params['Duration']], ymin, ymax, colors='black', linestyles=':')
    ax[2].set_ylim(ymin, ymax)

    ax[2].vlines(params['SilenceOnset'], ymax * .9, ymax, colors='black', linestyles='-', lw=0.25)
    # ax[2].set_title(f"{params['good_units'][unit]} - BG: {params['pairs'][pair][0][2:]} - "
    #              f"FG: {params['pairs'][pair][1][2:]}", fontweight='bold', size=12)
    ax[0].set_title(f"{params['good_units'][unit]}", fontweight='bold', size=12)
    xmin, xmax = ax[2].get_xlim()

    zscore, z_params = get_z(resp_idx, response, params)
    z_params['label'] = 'BG+FG Combo - Linear Sum'
    if sigma:
        ax[3].plot(x, sf.gaussian_filter1d(zscore[pair, unit, :], sigma), color='black', label=z_params['label'])
    if not sigma:
        ax[3].plot(x, zscore[pair, unit, :], color='black', label=z_params['label'])
    ax[3].hlines([0], x[0], x[-1], colors='black', linestyles=':', lw=0.5)
    ymin, ymax = ax[3].get_ylim()
    ax[3].set_ylim(ymin,ymax), ax[3].set_xlim(xmin, xmax)
    ax[3].vlines([0, params['Duration']], ymin, ymax, colors='black', linestyles=':', lw=0.5)
    ax[3].vlines(params['SilenceOnset'],ymax*.9,ymax,colors='black',linestyles='-', lw=0.25)
    ax[3].legend(loc='upper right', fontsize=6)

    ax[3].set_xlabel('Time (s)', fontweight='bold', size=10)
    ax[3].set_ylabel('Normalized Linear\nPrediction Error', fontweight='bold', size=10)

#spectrogram part
    folder_ids = [2, 3] # for tabor data
    bg_path = f"/auto/users/hamersky/baphy/Config/lbhb/SoundObjects/@OverlappingPairs/" \
              f"Background{folder_ids[0]}/{params['pairs'][pair][0]}.wav"
    fg_path = f"/auto/users/hamersky/baphy/Config/lbhb/SoundObjects/@OverlappingPairs/" \
              f"Foreground{folder_ids[1]}/{params['pairs'][pair][1]}.wav"
    ...
    ax[0].set_ylabel(f"BG: {params['pairs'][pair][0][2:]}", rotation=0, fontweight='bold',
                     size=8, labelpad=-35)
    ...
    ax[1].set_ylabel(f"FG: {params['pairs'][pair][1][2:]}", rotation=0, fontweight='bold',
                     size=8, labelpad=-35)

    ymin, ymax = ax[1].get_ylim()
    ax[0].vlines([0, 1*params['fs']], ymin, ymax, color='black', lw=0.5)
    ax[0].hlines([ymin+2,ymax], 0, 1*params['fs'], color='black', lw=0.5)
    ax[1].vlines([0, 1*params['fs']], ymin, ymax, color='black', lw=0.5)
    ax[1].hlines([ymin+1,ymax], 0, 1*params['fs'], color='black', lw=0.5)


def z_heatmaps_onepairs_figure(resp_idx, pair, response, params, tags=[],
                               sigma=None, arranged=False):
    """Plots a two column figure of subplots, one for each sound pair, displaying a heat map
    of the zscore for all the units."""
    tag_names = [params['good_units'][nn][-4:] for nn in tags]
    ...
    im = ax.imshow(zscore[pair, :, :], aspect='auto', cmap='RdBu_r',
              extent=[-0.5, (zscore[pair, :, :].shape[1] / params['fs']) -
                      0.5, zscore[pair, :, :].shape[0] - 0.5, -0.5], vmin=-abs_max, vmax=abs_max)
    # ax.set_title(f"Pair {pair}: BG {params['pairs'][pair][0]} - FG {params['pairs'][pair][1]}",
    ...
    cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
    fig.colorbar(im, cax=cbar_ax)
    ax.set_yticks(tagidx)
    ax.set_yticklabels(tag_names, fontweight='bold', fontsize=7)
