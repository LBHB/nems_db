...
def plot_binaural_psths(df, cellid, bg, fg, batch, save=False):
    ...
    df_filtered = df[(df.BG == bg) & (df.FG == fg) & (df.cellid == cell)]
    ...
    for e, a, c, s, l in zip(ep_num, ax_num, colors, styles, labels):
        ax[a].plot(time, sf.gaussian_filter1d(r_mean[epochs[e]], sigma=2)
                   * fs, color=c, linestyle=s, label=l)
    ...
    if save:
        site, animal, area, unit = cellid.split('-')[0], cellid[:3], df.area.loc[df.cellid == cell].unique()[0], cellid[8:]
