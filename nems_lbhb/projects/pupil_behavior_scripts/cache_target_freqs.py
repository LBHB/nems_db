...
df_307.to_csv(os.path.join(fpath, str(307), 'd_tar_freqs.csv'))
...
df_309.to_csv(os.path.join(fpath, str(309), 'd_tar_freqs.csv'))
...
df_313.to_csv(os.path.join(fpath, str(313), 'd_tar_freqs.csv'))

# ================================= batch 295 ====================================
perfile_df = pd.read_csv(os.path.join(fpath, str(295), 'd_afl_sdexp.csv'), index_col=0)
df_295 = pd.DataFrame()
...
for cellid in cells_295:
    sql = "SELECT sCellFile.stimfile, gData.svalue, gData.value, sCellFile.rawid from sCellFile INNER JOIN \
                ...
                AND sCellFile.cellid=%s order by sCellFile.rawid"
    d = nd.pd_query(sql, params=(cellid,))
    _, rawid = nd.get_stable_batch_cells(batch=295, cellid=cellid)
    d = d[d.rawid.isin(rawid)]
    pf_labels = np.unique([l for l in perfile_df[perfile_df['cellid']==cellid]['state_chan_alt'] if 'ACTIVE' in l])
...
df_295.to_csv(os.path.join(fpath, str(295), 'd_tar_freqs.csv'))
