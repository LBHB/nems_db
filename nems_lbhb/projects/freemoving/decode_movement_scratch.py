...
## load the recording
    ...
    est = rec.jackknife_mask_by_epoch(10,0,'TRIAL',invert=False)
    val = rec.jackknife_mask_by_epoch(10,0,'TRIAL',invert=True)
    ...
    for target_sig_name in target_sig_names:
        ...
        for i, target_chan in enumerate(est[target_sig_name].chans):
            ...
            layers = [
                WeightChannels(shape=(cellcount,5)),
                FIR(shape=(5,5)),
...
decoded_df = pd.DataFrame(decoded_df, columns=['siteid', 'target signal', 'target', 'target index', 'cc fit', 'cc test'])
