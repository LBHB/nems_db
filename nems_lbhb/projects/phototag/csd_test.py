...
## load the recording
parmfile = "/auto/data/daq/Clathrus/CLT028/CLT028c28_p_BNB.m"
...
chans = np.arange(384)
#continuous_data_list, timestamp0_list, selected_chans_xypos, selected_chans_str = \
...
rec = ex.get_recording(raw=True, resp=False, stim=False, recache=False, rawchans=None, rawhp=1, rawlp=100, rasterfs=200)

e = ep.epoch_names_matching(rec['raw'].epochs, "^STIM_")
stim_epoch = e[0]

r = rec['raw'].extract_epoch(stim_epoch)

plt.figure()
plt.imshow(r.mean(axis=0), origin='lower', interpolation='none')
