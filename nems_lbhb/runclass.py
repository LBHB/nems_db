...
def NAT_stim(exptevents, exptparams, stimfmt='gtgram', separate_files_only=False, channels=18, rasterfs=100, f_min=200, f_max=20000,
    ...
    :return:
        stim, tags, stimparam
    """
    runclass = exptparams.get('runclass','xx').split("_")
    if (len(runclass) > 1) and (runclass[1] == options["runclass"]):
        ReferenceClass = exptparams['TrialObject'][1]['TargetClass']
        ReferenceHandle = exptparams['TrialObject'][1]['TargetHandle'][1]
    else:
    ...
    if (ReferenceClass=='BigNat') & \
            (ReferenceHandle.get('TestBinaural','None').strip() != 'None'):
        # Binaural natural sounds
        sound_root = ReferenceHandle['SoundPath'].replace("\\", "/")
        ...
        #print(ReferenceHandle['Names'][:10])
    ...
    elif (ReferenceClass == 'BigNat') | (ReferenceClass == 'NaturalSounds') | (ReferenceClass == 'FerretVocal'):
        if ReferenceClass == 'BigNat':
            sound_root = ReferenceHandle['SoundPath'].replace("\\", "/")
            sound_root = sound_root.replace("H:/", "/auto/data/")
            sound_root = sound_root.replace("E:/sounds/v2", "/auto/data/sounds/BigNat/v2")
            sound_root = sound_root.replace("E:/", "/auto/data/")
            sound_root = Path(sound_root)
        elif ReferenceClass == 'FerretVocal':
            subset = ReferenceHandle['Subsets']
            if subset == 1:
                sound_root=Path(f'/auto/users/svd/code/baphy/SoundObjects/@FerretVocal/sounds')
            else:
                sound_root=Path(f'/auto/users/svd/code/baphy/SoundObjects/@FerretVocal/Sounds_set{subset}')
        else:
            subset = ReferenceHandle['Subsets']
            if subset == 1:
    ...
    elif ReferenceClass == 'PipSequence':
        raise ValueError(f"ReferenceClass {ReferenceClass} gtgram not supported.")
        # TODO: Finish support
        stimfilepath = baphy_stim_cachefile(exptparams, stimfmt='wav', rasterfs=40000, parmfilepath=None, **options)
    else:
        raise ValueError(f"ReferenceClass {ReferenceClass} gtgram not supported.")
    if len(stim_epochs) == 0:
        return {}, [], {}
