...
def baphy_load_stim(exptparams, parmfilepath, epochs=None, **options):
    ...
    if options['stimfmt'] in ['gtgram', 'nenv', 'lenv', 'wav']:
        ...
        stimfilepath1 = runclass.baphy_stim_cachefile(exptparams, parmfilepath, use_target=False, **options)
        stimfilepath2 = runclass.baphy_stim_cachefile(exptparams, parmfilepath, use_target=True, **options)
        ...
        stimfilepath = runclass.baphy_stim_cachefile(exptparams, parmfilepath, **options)
        ...
        if len(sortinfo[c]) and len(sortinfo[c][0]) >= sortidx + 1 and sortinfo[c][0][sortidx].size:
            ...
            for u in range(0, unitcount):
                ...
                if st.shape[0] == 3:
                    ...
                    log.info(f"BACKWARD COMPAT LOADER?? {u} {st.shape}")
                    unit_spike_events = np.array([])
                    for trialidx in range(1, TrialCount + 1):
                        ff = (st[0, :] == trialidx)
                        try:
                            this_spike_events = (st[1, ff] + Offset_spikefs[int(trialidx - 1)])
                        except:
                            import pdb
                            pdb.set_trace()
                        if len(comment) > 0:
                ...
                # designed only for cases where e.g. a single cellid doesn't spike during one
...
def get_mean_spike_waveform(cellid, usespkfile=None, load_from_db=True, save_to_db=True):
    ...
    if usespkfile is None:
        try:
            ...
            mwf = None
        if mwf is None:
            log.debug('failed. Trying in spike.npw files')
            try:
                mwf = usenpyfile()
            except:
                log.debug('failed. setting mwf=None')
                mwf = None
...
def get_spike_info(cellid=None, siteid=None, rawid=None, save_to_db=False):
    ...
    log.info(f"Compiling site info for {siteid}")
