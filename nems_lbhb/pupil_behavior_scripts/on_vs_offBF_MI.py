"""
Compare unique MI effects for on BF cells and off BF cells.
Tar freq can change per file, so for some cells will need to look 
at unique per file MI
"""
import pandas as pd
import numpy as np
...
for batch in batches:
    # load MI data
    dMI = pd.read_csv('nems_lbhb/pupil_behavior_scripts/d_{}_pup_fil.csv'.format(batch), index_col=0)

    # add column for unique pupil / file MI by subtracting shuffled MI
    p_merge = dMI[(dMI['state_sig']=='st.pup.fil') & (dMI.state_chan=='pupil')][['cellid', 'state_chan_alt', 'MI']].merge(\
                    dMI[(dMI['state_sig']=='st.pup0.fil') & (dMI.state_chan=='pupil')][['cellid', 'state_chan_alt', 'MI']], on=['cellid', 'state_chan_alt'])
    p_merge['pupil_unique'] = p_merge['MI_x'] - p_merge['MI_y']
    file_merge = dMI[dMI['state_sig']=='st.pup.fil'][['cellid', 'state_chan_alt', 'MI']].merge(\
                    dMI[dMI['state_sig']=='st.pup.fil0'][['cellid', 'state_chan_alt', 'MI']], on=['cellid', 'state_chan_alt'])
    file_merge['file_unique'] = file_merge['MI_x'] - file_merge['MI_y']
    dMI = dMI.merge(p_merge, on=['cellid']).merge(file_merge, on=['cellid'])
