...
outpath = "/auto/users/svd/docs/current/two_band_spn/"
...
df = nd.get_batch_cell_data(batch)
cellids=df.index.get_level_values(0).unique().tolist()

uri259_all=[]
for cellid in cellids:
    uri = generate_recording_uri(cellid, batch, loadkey=loadkey)
    uri259_all.append(uri)

files = [os.path.basename(u) for u in uri259_all]
df_VMN = pd.DataFrame({'cellid': cellids, 'file': files})
df_VMN.set_index(['cellid'], inplace=True)
...
for cellid in cellids:
    uri = generate_recording_uri(cellid, batch, loadkey=loadkey)
    uri289_all.append(uri)

files = [os.path.basename(u) for u in uri289_all]
df_NAT = pd.DataFrame({'cellid': cellids, 'file': files})
df_NAT.set_index(['cellid'], inplace=True)

uri259 = list(set(uri259_all))
uri289 = list(set(uri289_all))

outpath='/auto/users/svd/projects/two_chan_spn/VMN/'
for uri in uri259:
    print('cp {} {}'.format(uri, outpath))
outpath='/auto/users/svd/projects/two_chan_spn/NAT/'
for uri in uri289:
    print('cp {} {}'.format(uri, outpath))

outpath='/auto/users/svd/projects/two_chan_spn/'
df_NAT.to_csv(outpath+'NAT_data.csv')
df_VMN.to_csv(outpath+'VMN_data.csv')
