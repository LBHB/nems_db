...
class MainWindow(QtBaseClass, Ui_MainWindow):
    ...
    def __init__(self, ctx=None, xfspec=None, parent=None):
        ...
        self.ctx = ctx
        self.xfspec = xfspec
        self.input_name = ctx.get('input_name', 'stim')
        self.output_name = ctx.get('output_name', 'resp')
    ...
    def init_plots(self):
        ...
        self.outputPlot.update_plot(rec_name='val', signal_names=['pred', self.output_name], channels=0)

        for layer_area in self.plot_container.values():
            layer_area.update_plot()

        self.inputSpectrogram.plot_input(rec_name='val', sig_name=self.input_name)
